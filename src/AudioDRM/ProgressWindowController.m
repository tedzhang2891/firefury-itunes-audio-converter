...
- (void)convertWithTracks:(id)convertItems completeHandler:(void (^)(id, id))notifyAppController {
    ...
    for (id item in convertItems) {
        ...
        if (isRegister || itemTime <= 180.999f) {
            itemTime /= 1000;
        }
        else {
            itemTime = 180.0f;
    ...
    if ([self ensureEnoughDiskSpaceWithDuration:allTotalTime profile:profile]) {
        ...
        dispatch_async(global_queue, ^(void) {
            ...
            for (id track in convertTracks) {
                ...
                [metadata setObject:[NSNumber numberWithDouble:trackDuration] forKeyedSubscript:@"duration"];
                    ...
                    if (!bServerOpen) {
                        ...
                        if (!bServerOpen) {
                            dispatch_async(dispatch_get_main_queue(), ^(void){
                                NSBundle* mainBundle = [NSBundle mainBundle];
                                NSAlert* anAlert = [[NSAlert alloc] init];
                                anAlert.messageText = [mainBundle localizedStringForKey:@"Can not connect engine." value:nil table:nil];
                                anAlert.informativeText = [mainBundle localizedStringForKey:@"Some internal error happend, please retry it later." value:nil table:nil];
                                [anAlert addButtonWithTitle:[mainBundle localizedStringForKey:@"Ok" value:nil table:nil]];
                                [anAlert runModal];
                            });
                            break;
