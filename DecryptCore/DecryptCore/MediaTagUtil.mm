...
#if 1
        MP4FileHandle hAudio = MP4Modify(rawpath);
        if (hAudio) {
            ...
            MP4TagsSetName(mp4Tags, [title UTF8String]);
            
            NSString* artist = [metadata objectForKeyedSubscript:@"artist"];
            if (artist) {
                MP4TagsSetArtist(mp4Tags, [artist UTF8String]);
            }
            
            NSString* album = [metadata objectForKeyedSubscript:@"album"];
            if (album) {
                MP4TagsSetAlbum(mp4Tags, [album UTF8String]);
            }
            
            NSString* date = [metadata objectForKeyedSubscript:@"date"];
            if (date) {
                MP4TagsSetReleaseDate(mp4Tags, [date UTF8String]);
            }
            
            NSString* comment = [metadata objectForKeyedSubscript:@"comment"];
            if (comment) {
                MP4TagsSetComments(mp4Tags, [comment UTF8String]);
            }
            
            NSString* genre = [metadata objectForKeyedSubscript:@"genre"];
            if (genre) {
                MP4TagsSetGenre(mp4Tags, [genre UTF8String]);
            }
            
            NSString* albumartist = [metadata objectForKeyedSubscript:@"albumartist"];
            if (albumartist) {
                MP4TagsSetAlbumArtist(mp4Tags, [albumartist UTF8String]);
            }
            
            NSString* composer = [metadata objectForKeyedSubscript:@"composer"];
            if (composer) {
                MP4TagsSetComposer(mp4Tags, [composer UTF8String]);
            }
            
            NSString* track = [metadata objectForKeyedSubscript:@"track"];
            NSString* track_count = [metadata objectForKeyedSubscript:@"track count"];
            if (track) {
                const MP4TagTrack mp4TagTrack = {static_cast<uint16_t>([track intValue]), static_cast<uint16_t>([track_count intValue])};
                MP4TagsSetTrack(mp4Tags, &mp4TagTrack);
            }
            
            NSString* disc = [metadata objectForKeyedSubscript:@"disc"];
            NSString* disc_count = [metadata objectForKeyedSubscript:@"disc count"];
            if (disc) {
                const MP4TagDisk mp4TagDisk = {static_cast<uint16_t>([disc intValue]), static_cast<uint16_t>([disc_count intValue])};
                int discid = [disc intValue];
                MP4TagsSetDisk(mp4Tags, &mp4TagDisk);
            }
            
            if (imageArray)
            {
                for (id img in imageArray) {
                    MP4TagArtwork mp4TagArtwork = {const_cast<void*>([img bytes]), static_cast<uint32_t>([img length]), MP4_ART_JPEG};
                    MP4TagsAddArtwork(mp4Tags, &mp4TagArtwork);
                }
            }
            
            MP4TagsStore(mp4Tags, hAudio);
            MP4TagsFree(mp4Tags);
            MP4Close(hAudio);
        }
