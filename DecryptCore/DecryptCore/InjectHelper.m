...
+ (bool) isOnlyRecordMode
{
    ...
    if (OSVer.minorVersion <= 13 && (OSVer.majorVersion != 10 && OSVer.minorVersion != 10)) {
        return false;
    }
...
+ (int) injectorModeForBundle:(NSBundle*)b
{
    ...
    if ([bundleID isEqualToString:@"com.apple.iTunes"]) {
        ...
        NSArray* arrVer = [InjectHelper ITunesVersions:bundle];
        NSString* itunesVersion = [arrVer componentsJoinedByString:@"."];
        NSLog(@"itunes version is %@", itunesVersion);
        int major = [arrVer[0] intValue];
        int minor = [arrVer[1] intValue];
        int micro = [arrVer[2] intValue];
        int build = [arrVer[3] intValue];
        
        NSProcessInfo* procInfo = [NSProcessInfo processInfo];
        if (procInfo != nil) {
            NSOperatingSystemVersion OSVer = [procInfo operatingSystemVersion];
            if (major < 12 || minor >= 6) {
                mode = 4;
                if (![InjectHelper isOnlyRecordMode]) {
                    mode = 2;
