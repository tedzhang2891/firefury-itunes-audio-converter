...
+ (NSString*)converterLogPath {
    ...
    return [logFolder stringByAppendingPathComponent:@"iTunes.log"];
}
...
- (BOOL)connectToConvertCore {
    ...
    while (YES)
    {
        convertCore = [NSConnection rootProxyForConnectionWithRegisteredName:@"com.apple.iTunesConverter" host:NULL];
        if (convertCore)
            break;
        sleep(1);
        ...
        if ([converterCore convertFile:srcfile output:destfile stopDuration:duration convertSpeed:speed withProfile:profile contextInfo:signature]) {
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ...
                if ([sharedUserDefault boolForKey:@"PreferenceKeepChaptersKey"] &&
                    ...
                    NSArray* chapters = [ITunesChapterUtil chaptersFromITunesCurrentTrack];
                }
            ...
            while (YES) {
                while (YES) {
                    ...
                    previousDuration = convertedDuration;
                    convertedDuration = 0.0;
                    if (previousDuration >= 0.0) {
                        break;
                    }
                    usleep(USEC_PER_SEC / 5);
                    eachtime = [NSDate date];
                }
                elapsingTime = duration - previousDuration;
                if (elapsingTime == 0.0) {
                    NSDate* now = [NSDate date];
                    double timediff = [now timeIntervalSinceDate:eachtime];
                    if (timediff > elapsingTime) {
                        [converterCore stopConvert];
                        NSLog(@"duration %.3f, convert duration %.3f", duration, previousDuration);
                        break;
                    }
                }
                else {
                    if (handle) {
                        handle(previousDuration, &isAbort);
                        if (isAbort) {
                            break;
                        }
                    }
                    
                    if (timeleft != duration - previousDuration) {
                        timeleft = duration - previousDuration;
                        receive_times = 0.0;
                    }
                    else {
                        if (previousDuration != 0.0) {
...
- (id)init {
    if (self = [super init]) {
        self->musicAppIdentifer = @"com.apple.iTunes";
        //NSBundle* mainBundle = [NSBundle mainBundle];
        NSBundle* bundle = [NSBundle bundleForClass:[MusicConverter class]];
        NSString* pluginPath = [bundle pathForResource:@"ITunesConverterBundle" ofType:@"bundle"];
        self->pluginPath = pluginPath;
        NSUserDefaults* sharedUserDefault = [NSUserDefaults standardUserDefaults];
        NSInteger convertTimeout = [sharedUserDefault integerForKey:@"ConverterWaitTimeout"];
        NSInteger defaultTimeout = 20LL;
        if (convertTimeout)
